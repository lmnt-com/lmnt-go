...
// CheckTestServer reports whether the mock server at url is reachable. When it
// is not, the test is skipped if SKIP_MOCK_TESTS is set to a truthy value, and
// fails otherwise — so a missing mock server is a loud error locally but an
// opt-out skip in environments that cannot run Prism.
