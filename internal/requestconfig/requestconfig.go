...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": fmt.Sprintf("Lmnt/Go %s", internal.PackageVersion),
	}
}
...
func getPlatformProperties() map[string]string {
	return map[string]string{
		"X-Lmnt-Lang":            "go",
		"X-Lmnt-Package-Version": internal.PackageVersion,
		"X-Lmnt-OS":              getNormalizedOS(),
		"X-Lmnt-Arch":            getNormalizedArchitecture(),
		"X-Lmnt-Runtime":         "go",
		"X-Lmnt-Runtime-Version": runtime.Version(),
	}
}
...
func NewRequestConfig(ctx context.Context, method string, u string, body any, dst any, opts ...RequestOption) (*RequestConfig, error) {
	...
	req.Header.Set("X-Lmnt-Retry-Count", "0")
	req.Header.Set("X-Lmnt-Timeout", "0")
	for k, v := range getDefaultHeaders() {
		req.Header.Add(k, v)
	}
	req.Header.Set("lmnt-version", internal.APIVersion)
	...
	if req.Header.Get("X-Lmnt-Timeout") == "0" {
		if cfg.RequestTimeout == time.Duration(0) {
			req.Header.Del("X-Lmnt-Timeout")
		} else {
			req.Header.Set("X-Lmnt-Timeout", strconv.Itoa(int(cfg.RequestTimeout.Seconds())))
		}
	}
...
type RequestConfig struct {
	...
	Body         io.Reader
	// WSDialer overrides how the generated WebSocket session resource opens its
	// connection. nil means use wsstream.Dial. Set by option.WithWSDialer; used
	// for test injection. (LMNT addition, not part of the Stainless runtime.)
	WSDialer wsstream.Dialer
	// WSKeepAlive controls the WebSocket session keepalive ping interval. The
	// zero value means use the default; a negative value disables keepalive.
	// Set by option.WithWSKeepAlive. (LMNT addition.)
...
func (cfg *RequestConfig) Execute() (err error) {
	...
	shouldSendRetryCount := cfg.Request.Header.Get("X-Lmnt-Retry-Count") == "0"
	...
	for retryCount := 0; retryCount <= cfg.MaxRetries; retryCount += 1 {
		...
		if shouldSendRetryCount {
			req.Header.Set("X-Lmnt-Retry-Count", strconv.Itoa(retryCount))
		}
...
// SessionConfig applies opts to a fresh RequestConfig suitable for opening a
// WebSocket session: it carries a usable Request so header-setting options
// (e.g. option.WithAPIKey) apply, but it makes no HTTP call. The generated
// WebSocket session constructor reads BaseURL, APIKey, the request headers,
// WSDialer, and WSKeepAlive off the returned config. (LMNT addition.)
func SessionConfig(ctx context.Context, opts ...RequestOption) (*RequestConfig, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://lmnt.invalid", nil)
	if err != nil {
		return nil, err
	}
	cfg := &RequestConfig{
