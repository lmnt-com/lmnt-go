...
// RequestOption is an option for the requests made by the lmnt API Client
...
// passed to lmnt.NewClient, causes it to skip the environment-based
// credential autoload (LMNT_API_KEY, LMNT_BASE_URL). The hardcoded
...
// [WithoutEnvironmentDefaults] marker. Used by lmnt.NewClient to decide
// whether to prepend lmnt.DefaultClientOptions.
...
// WithWSDialer returns a RequestOption that overrides how the generated
// WebSocket session resource opens its connection. Used to inject an
// in-process fake [wsstream.Conn] in tests; production code never needs it.
func WithWSDialer(dialer wsstream.Dialer) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
		r.WSDialer = dialer
		return nil
	})
...
// WithWSKeepAlive returns a RequestOption that sets the keepalive ping interval
// for WebSocket sessions. The default is 30s; a non-positive duration disables
// keepalive pings.
func WithWSKeepAlive(interval time.Duration) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
		r.WSKeepAlive = interval
		return nil
	})
}
